...
namespace Royal_Shooting_range.GameClasses
{
    public class PauseMenu
    {
        ...
        public void Draw(Graphics g, bool onPause)
        {
            if (onPause)
            {
                g.FillEllipse(new SolidBrush(Color.Aqua), Game.WidthScreen / 2 - size.Width / 2, Game.HeightScreen / 2 - size.Height / 2, size.Width, size.Height);
                Update(g, onPause);
            }
        }
    }
