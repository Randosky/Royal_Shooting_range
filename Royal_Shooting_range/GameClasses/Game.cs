        ...
        public void Update(bool isSpacePressed, bool isSecondTime, Level curLevel)
        {
            ...
            foreach (var barrier in barriers)
            {
                var conditionBarrier = arrow.x + arrow.scale.Width <= barrier.x + 20 &&
                                       ...
                                       arrow.y <= barrier.y + barrier.scale.Height + 20 &&
                                       arrow.y > barrier.y - 30;
                if (conditionBarrier)
                {
                    barrier.stackedArrow = new Arrow(arrow.x + 30, arrow.y + 7 * (arrow.angle / 10), arrow.angle, 0.2f);
                    Annulate();
                }
            }
            ...
            if (player.currFrame / 8 == 8 && player.currAnimation == 0)
                IsEightFrame = true;
